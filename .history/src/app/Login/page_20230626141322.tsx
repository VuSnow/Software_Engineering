...
const Login = ({ setShowLoginModal }: ModalProps) => {
  return (
    <div className="loginModal fixed w-full h-full top-0 left-0 z-[150]">
      ...
      >
        <span className="close-popup cursor-pointer absolute top-3 right-4 text-2xl font-bold">
          X
        </span>
        <div>
          <div className="form-container">
            <form id="login-form">
              ...
              <button className="login-facebook bg-[#3B5998] text-white border border-solid border-[#ccc] w-full text-center h-10 rounded-2xl cursor-pointer mb-4 flex">
                <span className="title">Đăng nhập với Facebook</span>
                <div className="icon w-[18px]">
                  <Image src={facebookIcon} alt="facebook icon" />
                </div>
              </button>
              <button className="login-google border border-solid border-[#ccc] w-full text-center h-10 rounded-2xl cursor-pointer mb-4">
